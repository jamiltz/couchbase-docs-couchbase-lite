...
:url-api: https://docs.couchbase.com/mobile/1.4.1/couchbase-lite-net/html/6c9c1563-1e89-6817-d451-c6c73df96aec.htm
...
Refer to the <<revision,Revisions>> guide to learn about compaction and pruning to manage the database size.
...
Refer to the https://developer.couchbase.com/documentation/mobile/1.4/training/develop/adding-synchronization/index.html#resolve-conflicts[Adding Synchronization] lesson to learn how to resolve conflicts in your application.
...
image::compaction.gif[]

Compaction can only be invoked manually via the https://developer.couchbase.com/documentation/mobile/1.4/references/couchbase-lite/couchbase-lite/database/database/index.html#void-compact[compact()] method.
...
`endKey`::
The last key to return.
The default value, `null`, means to continue to the end.
`descending`::
If set to `true`, the keys will be returned in reverse order.
...
`limit`::
If nonzero, this is the maximum number of rows that will be returned.
`skip`::
If nonzero, this many rows will be skipped (starting from the `startKey` if any).

Some more advanced properties that aren't used as often:

`keys`::
If provided, the query will fetch only the rows with the given keys (and `startKey` and `endKey` will be ignored).
`startKeyDocID`::
If multiple index rows match the startKey, this property specifies that the result should start from the one(s) emitted by the document with this ID, if any.
(Useful if the view contains multiple identical keys, making .startKey ambiguous.)
`endKeyDocID`::
If multiple index rows match the endKey, this property specifies that the result should end with from the one(s) emitted by the document with this ID, if any.
(Useful if the view contains multiple identical keys, making .startKey ambiguous.)
`indexUpdateMode`::
Changes the behavior of index updating.
...
`mapOnly`::
If set to true, prevents the reduce function from being run, so you get all of the index rows instead of an aggregate.
...
`includeDeleted`::
In this mode, deleted documents are included as well.
`showConflicts`::
In this mode, each ``QueryRow```'s `conflictingRevisions` property can be used to find whether it's in conflict and what the IDs of the conflicting revisions are.
`onlyConflicts`::
Like `showConflicts`, but _only_ conflicted documents are returned.
...
Push vs Pull::
A push replication uploads changes from the local database to the remote one;
a pull downloads changes from the remote database to the local one.
One-shot vs Continuous::
By default a replication runs long enough to transfer all the changes from the source to the target database, then quits.
...
Filtered::
Replications can have filters that restrict what documents they'll transfer.
...
There is an incompatibility in the way the `filter` parameter is handled in the `+POST /{db}/_changes+` request (see {url-issues-ios}/1139[#1139]).

==== Filtering by document IDs

In one-shot *pull* replications with Sync Gateway, it's possible to specify a list of document IDs (this feature is not available for replications in continuous mode, see {url-issues-sync}/1703[#1703]).
...
Refer to the xref:index.adoc[installation guide] of the platform of your choice to install the Listener component.
...
Type::
Bonjour can be used by many other types of devices on the LAN (printers, scanners, other apps etc.).
The service type is a way to interact only with peers whose service type is the same.
Name::
A string to serve as identifier for other peers.
It should be unique for each peer.
It does not need to be human-readable.
Port::
The port number the Listener is running on.
Metadata::
Optional data that will be sent in the advertizment packets (the size limit is around 1.5KB).
...
Bonjour for iOS and Mac applications::
The Couchbase Lite SDK exposes part of the Bonjour API for an easier integration.
...
NSD for Android applications::
The de facto framework for Android is called Network Service Discovery (NSD) and is compatible with Bonjour since Android 4.1.
...
JmDNS::
Implementation in Java that can be used in Android and Java applications (https://github.com/jmdns/jmdns[official repository]).
...
It may be desirable to use <<filtered-replications,filter functions>> to replicate only the documents of interest to another peer.
...
* {url-issues-net}/827[#827] Purge does not remove row from 'docs' table
* {url-issues-net}/856[#856] View indexing is very slow, due to mis-optimized SQLite query

.Enhancements
* {url-issues-net}/867[#867] Preserve revision history for each "branch" on pruning
* {url-issues-net}/915[#915] Support DocIDs setting for pull replication with Sync Gateway

.Bugs
* {url-issues-net}/778[#778] Push sync becomes slower as local DB grows, fails to advance lastSequence
* {url-issues-net}/821[#821] .NET not encoding atts_since
* {url-issues-net}/824[#824] Pull replication stopping when coming online
* {url-issues-net}/830[#830] Remove RunInOuterTransaction
* {url-issues-net}/832[#832] New_edits on _bulk_docs should default to true
* {url-issues-net}/834[#834] 1.4 breaks custom headers on PUT HTTP requests
* {url-issues-net}/835[#835] Unable to select multiple values with LINQ 1.4
* {url-issues-net}/840[#840] Couchbase.Lite.dll missing Product Version Info and Others
* {url-issues-net}/847[#847] Trying to remove encryption from a SQLite DB throws an exception
* {url-issues-net}/848[#848] Retry loop causes web socket changes feed to stop
* {url-issues-net}/850[#850] Wrong return code for listener causes problems with puller and deleted items.
* {url-issues-net}/853[#853] Updating a design doc view map function does not take effect
* {url-issues-net}/863[#863] PollInterval is not obeyed
* {url-issues-net}/910[#910] Channel removal potentially disrupts attachments
* {url-issues-net}/914[#914] Bulk download error not passed to client
...
.Enhancements
* {url-issues-net}/245[#245] Implement LINQ IQueryable
* {url-issues-net}/575[#575] Package Custom SQLite
* {url-issues-net}/709[#709] Implement LiteServ on mobile platforms
* {url-issues-net}/741[#741] Remove Mono.Security build
* {url-issues-net}/745[#745] Support 'stale=false' queries in Listener
* {url-issues-net}/766[#766] Push replication does not work when attachment content type is missing
* {url-issues-net}/797[#797] Add DatabaseChangedEventArgs.IsDeletion to public API
* {url-issues-net}/802[#802] Attempt to silence websockets when they shutdown

.Bugs
* {url-issues-net}/672[#672] .NET LiteServ is loading windows specific binaries on mac
* {url-issues-net}/710[#710] SQL Exception caused by trying to query non-existent `+maps_#+` table
* {url-issues-net}/713[#713] QueryOptions skip and limit used incorrectly during reduce
* {url-issues-net}/718[#718] V1.3 Continuous replication stops on change tracker error
* {url-issues-net}/719[#719] Replication - Inconsistent Number of Documents After Replicating
* {url-issues-net}/724[#724] V1.3 GetPendingDocumentIDs value is cached while offline for Push
* {url-issues-net}/725[#725] Replication fails to authenticate with POST _replicate cookie.
* {url-issues-net}/727[#727] BlobStoreWriter fails to delete encryption marker when removing encryption
* {url-issues-net}/731[#731] Missing docs when pushing to multiple sync gateways
* {url-issues-net}/732[#732] Transaction rollback does not invalidate cached doc ID map
* {url-issues-net}/735[#735] SIGSEGV when closing a database while replication is still active
* {url-issues-net}/742[#742] CB Lite on Android Xamarin fails to replicate when local DB is large
* {url-issues-net}/749[#749] Attachments failed to replicate when deleted and recreated
* {url-issues-net}/753[#753] SQLiteStorageEngine can return stale data in rare cases
* {url-issues-net}/755[#755] Pull replication gets stuck in Active with more completed changes than pending changes
* {url-issues-net}/756[#756] Pusher does not report "Forbidden" errors
* {url-issues-net}/757[#757] Replicator should not assume LastSequence is an integer
* {url-issues-net}/765[#765] Not all docs replicating with p2p .NET pull replication
* {url-issues-net}/767[#767] Changes feed skipping changes on rapid long poll
* {url-issues-net}/772[#772] ManagementException in GetWindowsName() of Platform.cs
* {url-issues-net}/773[#773] BrowseService's Dispose crash fix not merged to Master?
* {url-issues-net}/776[#776] Replication.Stop() doesn't release sockets
* {url-issues-net}/779[#779] Attachments disappearing from docs when continuous pull is enabled
* {url-issues-net}/782[#782] Encrypted attachments fail to push
* {url-issues-net}/785[#785] NullReferenceException in Replication.GetStatusFromError
* {url-issues-net}/789[#789] Investigate root cause of deletion not propagating to .NET client
* {url-issues-net}/795[#795] Set ServicePointManager.SecurityProtocol
* {url-issues-net}/796[#796] Headers not sent when pullReplication.Continuous = true
* {url-issues-net}/803[#803] NullReferenceException in WebSocketChangeTracker
* {url-issues-net}/804[#804] Attachment doesn't handle non-seekable streams gracefully
* {url-issues-net}/806[#806] 1.4 SQLite returns incorrect reason on REST API Get to purged doc
* {url-issues-net}/807[#807] Handle closed web sockets by reconnecting
* {url-issues-net}/825[#825] Pull replication doesn't restart with web sockets
* {url-issues-net}/826[#826] .NET not sending all docs to a client pull replication (p2p, SSL)
